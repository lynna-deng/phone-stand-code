function [transI2_cropped, I2_registered_name] = stereoCam_alignImages(reference_photo, ...
    unaligned_photo, registration_filetype, selectMask, reference_photo_mask, ...
    ...
    %   selectMask: True if a new mask is to be selected, false if not
    ...
    elseif options.DetectionMethod == "Manual" && ~isempty(reference_photo_GCP) ...
        ...
        % Load previously selected keypoints
    ...
    plot(I1_matchedPoints_inliers.Location(:, 1), I1_matchedPoints_inliers.Location(:, 2), 'ro');
    hold on
    if selectMask == true || (~isempty(reference_photo_mask) && ~isempty(unaligned_photo_mask))
    ...
    [~, I2_filename, ~] = fileparts(unaligned_photo);
    I2_registered_name = append(I2_filename, '_registered', registration_filetype);
